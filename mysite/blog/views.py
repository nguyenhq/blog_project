...
from django.contrib.auth.mixins import LoginRequiredMixin # When using class-based views, you can achieve the same behavior as with login_required by using the LoginRequiredMixin
...
def comment_approve(request, pk):
    comment = get_object_or_404(Comment, pk=pk) #Comment's model
    comment.approve() #call function in model.py
    return redirect('post_detail', pk=comment.post.pk)

@login_required
def comment_remove(request,pk):
    comment = get_object_or_404(Comment, pk=pk) #get comment
...
#######################################
def changeDate(date):
    return date_use.translate(
        dateTime = datetime.date(2013,)
    )
